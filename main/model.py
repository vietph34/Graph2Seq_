...
class Graph2SeqNN(object):
    ...
    def __init__(self, mode, conf, path_embed_method):
        ...
        self.fw_adj_info = tf.placeholder(tf.int32, [None, None])            
        self.bw_adj_info = tf.placeholder(tf.int32, [None, None])             
        self.feature_info = tf.placeholder(tf.int32, [None, None])          
        self.batch_nodes = tf.placeholder(tf.int32, [None, None])            
    ...
    def encode(self):
        ...
        with tf.variable_scope("graph_encoding_layer"):
            ...
            node_embedding = tf.nn.embedding_lookup(self.word_embeddings, self.feature_info)
            node_embedding = tf.reshape(node_embedding, [tf.shape(node_embedding)[0], -1]) # batch_size, nnodes, embedding_dim

            # neighbor sampling
            fw_sampler = NeighborSampler(self.fw_adj_info)
            bw_sampler = NeighborSampler(self.bw_adj_info)

            nodes = tf.reshape(self.batch_nodes, [-1, ])
            sampled_fw = fw_sampler((nodes, self.sample_size_per_layer))
            sampled_bw = bw_sampler((nodes, self.sample_size_per_layer))
            fw_sampled_neighbors_len = tf.constant(0)
            bw_sampled_neighbors_len = tf.constant(0)           

            # nnodes, embedding_dim
            fw_hidden = tf.nn.embedding_lookup(node_embedding, nodes)
            bw_hidden = tf.nn.embedding_lookup(node_embedding, nodes)
            
            # aggregate information from k-hop neighbors
            for layer in range(self.sample_layer_size):
                ...
                # [nndoes, adj, embedding_dim]
                if layer == 0:
                    # initialize neighbor representation for the first layer
                    neigh_hidden = tf.nn.embedding_lookup(node_embedding, sampled_fw)
                    tmp_sum = tf.reduce_sum(tf.nn.relu(neigh_hidden), axis=2)
                    tmp_mask = tf.sign(tmp_sum)
                    fw_sampled_neighbors_len = tf.reduce_sum(tmp_mask, axis=1)

                else:
                    neigh_hidden = tf.nn.embedding_lookup(
                        tf.concat([fw_hidden, tf.zeros([1, dim_mul * self.hidden_layer_dim])], 0), sampled_fw)

                fw_hidden = fw_aggregator((fw_hidden, neigh_hidden, fw_sampled_neighbors_len))
                ...
                if layer == 0:
                    # initialize neighbor representation for the first layer
                    neigh_hidden = tf.nn.embedding_lookup(node_embedding, sampled_bw)
                    tmp_sum = tf.reduce_sum(tf.nn.relu(neigh_hidden), axis=2)
                    tmp_mask = tf.sign(tmp_sum)
                    ...
                    neigh_hidden = tf.nn.embedding_lookup(
                        tf.concat([bw_hidden, tf.zeros([1, dim_mul * self.hidden_layer_dim])], 0), sampled_bw)

                bw_hidden = bw_aggregator((bw_hidden, neigh_hidden, bw_sampled_neighbors_len))

            # combine information from two direction
            ...
            source_sequence_length = tf.reshape(
                tf.ones([tf.shape(hidden)[0], 1], dtype=tf.int32) * self.single_graph_nodes_size,
                (tf.shape(hidden)[0],))

            return hidden, graph_embedding, source_sequence_length
